...
```{r}
# ----------------------------------------------
# Permutation‐null correlation check + residuals
# ----------------------------------------------

# 1) Observed correlations (lower triangle)
...
k <- 1
for(i in 1:nperm) {
  Xp <- apply(X, 2, sample)      # permute each column
  Cp <- cor(Xp)
...
# 4) Fit 1‐factor reconstruction (using MoM‐Trio estimates)
X_recon <- outer(b_trio, lambda_trio) 
resid_mat <- X - X_recon
R_resid   <- cor(resid_mat)

# 5) Histogram of residual correlations
hist(R_resid[lower.tri(R_resid)], breaks = 50, freq = FALSE,
     col   = rgb(0,1,0,0.3),
     xlim  = c(-1,1),
     main  = "Residual Correlations after 1‐Factor Fit",
