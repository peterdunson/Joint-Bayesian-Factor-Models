...
# Pick a few subjects to visualize (e.g., first 5)
subjects_to_plot <- 1:10

# Observed: actual values for each variable
# Predicted: b_trio[i] * lambda_trio for each p
par(mfrow = c(length(subjects_to_plot), 1), mar = c(3,4,2,1))
for (i in subjects_to_plot) {
  obs   <- X[i, ]                # observed (length P)
  pred  <- b_trio[i] * lambda_trio  # predicted (length P)
