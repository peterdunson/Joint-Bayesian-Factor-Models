...
fit_mgsp    <- readRDS("fits/fit_mgsp_k1_cipsim.rds")
...
hist(vdiff_all, breaks = 50,
     main = "MGSP: Distribution of λ[p]^2·C[q,r] – C[p,q]C[p,r]",
...
```{r}
# # ----------------------------------------------
# # Combined density plot: permutation null, observed, and residual correlations
# # ----------------------------------------------
# 
# # 1) Compute observed correlations
# obs_corrs <- C[lower.tri(C)]
# 
# # 2) Generate permutation‐null correlations
# nperm      <- 1000
# perm_corrs <- replicate(nperm, {
#   Xp <- apply(X, 2, sample)
#   cor(Xp)[lower.tri(C)]
# })
# perm_corrs <- as.numeric(perm_corrs)
# 
# # 3) Compute residual correlations after 1‐factor MoM fit
# X_recon   <- outer(b_trio, lambda_trio)
# R_resid   <- cor(X - X_recon)
# resid_corrs <- R_resid[lower.tri(R_resid)]
# 
# # 4) Estimate densities
# d_perm  <- density(perm_corrs,   from = -1, to = 1)
# d_obs   <- density(obs_corrs,    from = -1, to = 1)
# d_resid <- density(resid_corrs,  from = -1, to = 1)
# 
# # 5) Plot all three together
# plot(d_perm, 
#      main = "Correlation Distributions: Null vs Observed vs Residual",
#      xlab = "Correlation",
#      lwd  = 2,
#      ylim = range(d_perm$y, d_obs$y, d_resid$y))
# lines(d_obs,   col = "lightblue",        lwd = 2)
# lines(d_resid, col = "forestgreen", lwd = 2)
# legend("topright",
#        legend = c("Permutation null", "Observed", "Residual"),
#        col    = c("black", "lightblue", "forestgreen"),
#        lwd    = 2,
#        bty    = "n")
