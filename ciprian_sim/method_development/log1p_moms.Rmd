...
plot_hist_with_null(offdiag_before_trio, offdiag_null, "MoM-Trio: Before", col="dodgerblue")
plot_hist_with_null(offdiag_before_mgsp, offdiag_null, "MGSP: Before", col="dodgerblue")
plot_hist_with_null(offdiag_before_hs, offdiag_null, "Horseshoe: Before", col="dodgerblue")
plot_hist_with_null(offdiag_before_ssl, offdiag_null, "Spike-and-Slab: Before", col="dodgerblue")
par(mfrow = c(1,1))
...
B <- 500 # Number of permutation null draws
cat("\n===== DSC Distance to Permutation Null (Z-Fisher) =====\n")

print_dsc_results <- function(name, dsc_obs, dsc_null) {
  cat(sprintf(
    "%-15s | Observed DSC = %.3f | Null Mean DSC = %.3f | Null SD = %.3f\n",
    name, dsc_obs, mean(dsc_null), sd(dsc_null)
  ))
}

# MoM-Trio
dsc_trio_before <- dsc_with_permutation_null_obs(Y, B = B)
print_dsc_results("MoM-Trio: BEFORE", dsc_trio_before$dsc_obs, dsc_trio_before$dsc_null)
dsc_trio_after <- dsc_with_permutation_null_obs(Y_resid_trio, B = B)
print_dsc_results("MoM-Trio: AFTER", dsc_trio_after$dsc_obs, dsc_trio_after$dsc_null)

# MGSP
dsc_mgsp_before <- dsc_with_permutation_null_obs(Y, B = B)
print_dsc_results("MGSP: BEFORE", dsc_mgsp_before$dsc_obs, dsc_mgsp_before$dsc_null)
dsc_mgsp_after <- dsc_with_permutation_null_obs(Y_resid_mgsp, B = B)
print_dsc_results("MGSP: AFTER", dsc_mgsp_after$dsc_obs, dsc_mgsp_after$dsc_null)

# Horseshoe
dsc_hs_before <- dsc_with_permutation_null_obs(Y, B = B)
print_dsc_results("Horseshoe: BEFORE", dsc_hs_before$dsc_obs, dsc_hs_before$dsc_null)
dsc_hs_after <- dsc_with_permutation_null_obs(Y_resid_hs, B = B)
print_dsc_results("Horseshoe: AFTER", dsc_hs_after$dsc_obs, dsc_hs_after$dsc_null)

# Spike-and-Slab
dsc_ssl_before <- dsc_with_permutation_null_obs(Y, B = B)
print_dsc_results("Spike-and-Slab: BEFORE", dsc_ssl_before$dsc_obs, dsc_ssl_before$dsc_null)
dsc_ssl_after <- dsc_with_permutation_null_obs(Y_resid_ssl, B = B)
print_dsc_results("Spike-and-Slab: AFTER", dsc_ssl_after$dsc_obs, dsc_ssl_after$dsc_null)
